...
const PaywallModal = ({ open, onClose, subtitle, onUpgrade }) => {
    ...
    <Modal open={open} onClose={onClose} title="Upgrade">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
          <Btn onClick={handleUpgrade} full loading={loading}>
            <Icon name="lock" size={14} color="#fff" /> Desbloquear agora
          </Btn>
          <Btn variant="ghost" onClick={onClose} full disabled={loading}>
            Continuar em modo de teste
          </Btn>
        </div>
        <p style={{ fontSize: 11, color: 'var(--text-light)', textAlign: 'center' }}>
          Checkout: {CHECKOUT_URL === 'dynamic-checkout' ? 'sessao Stripe dinamica' : CHECKOUT_URL}
        </p>
      </div>
    </Modal>
  )
